...
namespace olympia::coreutils
{
    ...
    // Get the extension 'issue_queue' from the CoreExtension
    // extension.  The node passed is expected to be the node
    // containing the extension (for example, core0)
    inline auto getIssueQueueTopology(sparta::TreeNode * node)
    {
        auto core_extension           = node->getExtension(olympia::CoreExtensions::name);
        auto core_extension_params    = sparta::notNull(core_extension)->getParameters();
        auto issue_queue_topology_param = sparta::notNull(core_extension_params)->getParameter("issue_queue_topology");
        return sparta::notNull(issue_queue_topology_param)->getValueAs<olympia::CoreExtensions::IssueQueueTopology>();
    }
